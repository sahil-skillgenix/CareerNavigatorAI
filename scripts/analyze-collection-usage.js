  ...
  try {
    ...
    for (const file of modelFiles) {
      ...
      try {
        ...
        const collectionMatch1 = content.match(/mongoose\.model\([^,]+,[^,]+,\s*["']([^"']+)["']/g);
        
        // Pattern 2: mongoose.model("ModelName", schema)
        const collectionMatch2 = content.match(/mongoose\.model\(["']([^"']+)["'],\s*\w+Schema\)/g);
        
        // Extract the name from pattern 1 matches
        if (collectionMatch1) {
          for (const match of collectionMatch1) {
            try {
              ...
              console.log(`Error parsing match in ${file}: ${match}`);
            }
          }
        }
        
        // Extract the name from pattern 2 matches
        if (collectionMatch2) {
          for (const match of collectionMatch2) {
            try {
              const modelNameMatch = match.match(/["']([^"']+)["']/);
              if (modelNameMatch && modelNameMatch[1]) {
                const modelName = modelNameMatch[1];
                // When the collection name is not explicit, MongoDB uses the model name (lowercased and pluralized)
                // But for consistency purposes, we've been standardizing with prefixes, so look for both options
              ...
              console.log(`Error parsing model name in ${file}: ${match}`);
            }
          }
        }
      } catch (error) {
