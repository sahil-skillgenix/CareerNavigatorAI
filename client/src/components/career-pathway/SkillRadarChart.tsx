...
const extractTopSkills = (results: CareerAnalysisResult): any[] => {
  try {
    // Handle potential missing data structures
    if (!results.skillMapping) {
      console.error('SkillRadarChart: Missing skillMapping in results');
      return [];
    }
    
    if (!results.skillGapAnalysis) {
      console.error('SkillRadarChart: Missing skillGapAnalysis in results');
      return [];
    }
    
    // Initialize allSkills as an empty array
    let allSkills: Array<{
      name: string;
      currentLevel: number;
    ...
    // Add SFIA 9 skills if available
    ...
    // Add DigComp 2.2 skills if available
    ...
    // Extract skills from gaps for required level if available
    if (Array.isArray(results.skillGapAnalysis.gaps)) {
    ...
    // Extract skills from strengths for current level if available
    if (Array.isArray(results.skillGapAnalysis.strengths)) {
    ...
    // If no skills were found, provide a minimal set of placeholder skills based on desired role
    if (allSkills.length === 0) {
      console.warn('SkillRadarChart: No skills found in data, looking for fallback data');
      
      // Check if we can get skills from other parts of the result
      if (results.developmentPlan && Array.isArray(results.developmentPlan.skillsToAcquire)) {
        allSkills = results.developmentPlan.skillsToAcquire.map(skill => ({
          name: typeof skill === 'string' ? skill : skill.name || skill.skill || 'Unknown Skill',
          currentLevel: 1,
          requiredLevel: 3,
    ...
    const topSkills = allSkills.slice(0, 6).map(skill => ({
      skill: skill.name,
    ...
    console.log('SkillRadarChart: Extracted skills successfully', {
    ...
    console.error('SkillRadarChart: Error extracting skills', error);
    return [];
  }
};
